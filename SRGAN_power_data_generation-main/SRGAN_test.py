...
def model_inference(data_type, input_resolution, output_resolution, evaluation_data_file_name, FILENAME, load_pre_trained_model=True):
    '''
    ...
    X_test_lr = get_evaluation_data(data_type, input_resolution, evaluation_data_file_name, FILENAME)
...
if __name__ == '__main__':
    ...
    for filename in os.listdir(data_dir):
        ...
        if os.path.isfile(f):
            ...
            print(f)
            gen_data = model_inference(data_type, input_resolution, output_resolution, evaluation_data_file_name, FILENAME)

            print(type(gen_data))
            print(len(gen_data))
            print(len(gen_data[0]))
            print(gen_data)



            np.save('results/'+FILENAME + 'result.npy', gen_data)
